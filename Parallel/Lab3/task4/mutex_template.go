...
func (t *Process_Task_Type) Start(printerChan chan Traces_Sequence_Type, Wait_for_Finish *sync.WaitGroup) {
	...
	for step := 0; step < t.Nr_of_Steps/4; step++ {
		...
		atomic.StoreInt32(&Wants_To_Enter[t.Process.Id], 1)
		for atomic.LoadInt32(&Wants_To_Enter[t.Process.Other_Id]) == 1 {
			if atomic.LoadInt32(&Turn) != int32(t.Process.Id) {
				atomic.StoreInt32(&Wants_To_Enter[t.Process.Id], 0)
				for atomic.LoadInt32(&Turn) == int32(t.Process.Other_Id) {
				}
				atomic.StoreInt32(&Wants_To_Enter[t.Process.Id], 1)
			}
		}
		...
		atomic.StoreInt32(&Wants_To_Enter[t.Process.Id], 0)
		atomic.StoreInt32(&Turn, int32(t.Process.Other_Id))
		time.Sleep(10) // to avoid same times in out
