  ...
  begin
    for I in 1 .. Nr_Of_Travelers + Nr_Of_Wild_Travelers loop
      accept Report(Traces : Traces_Sequence_Type) do
        Print_Traces(Traces); -- it's not in order. It just waits until every thread has requested print
      end Report;
    end loop;

    Finish := True;

    for I in 1 .. Nr_Of_Traps loop -- we gather traps reports after others do the same
      accept Report(Traces : Traces_Sequence_Type) do
        Print_Traces(Traces);
      end Report;
    end loop;
